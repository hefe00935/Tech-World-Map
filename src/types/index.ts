...
export interface InternetSilkroad {
  id: string;
  name: string;
  points: [number, number][];
  routeType: 'terrestrial' | 'submarine' | 'mixed';
  capacityTbps?: number;
  description?: string;
}

export type CableAdvisorySeverity = 'fault' | 'degraded' | 'cyber';
...
export interface Satellite {
  ...
  lon: number;
  alt: number;
  type: 'comm' | 'spy' | 'scientific' | 'other';
  owner: string;
  lastUpdated: Date;
}
...
export interface CyberAttack {
  ...
  attackType: 'ddos' | 'malware' | 'phishing' | 'exploit';
...
export interface MapLayers {
  ...
  techHQs?: boolean;
  techEvents?: boolean;
  // Cyber variant layers
  // Cyber variant layers
  cyberAttacks?: boolean;
  satellites?: boolean;
}

export interface StrategicDataCenter {
  id: string;
  name: string;
  owner: string;
  country: string;
...
// Backwards-compatible type aliases for legacy AI naming
export type AIDataCenter = StrategicDataCenter;

// Backwards-compatible alias: reuse StrategicRegulation shape for AIRegulation
export type AIRegulation = StrategicRegulation;
...
export interface StrategicRegulation {
  id: string;
  name: string;
  shortName: string;
  country: string;
...
export interface IntelResearchLab {
  id: string;
  name: string;
  lat: number;
  lon: number;
