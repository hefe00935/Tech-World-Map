...
export class CircuitBreaker<T> {
  ...
  recordFailure(error?: string): void {
    ...
    if (this.state.failures >= this.maxFailures) {
      ...
      Logger.warn(`[${this.name}] On cooldown for ${this.cooldownMs / 1000}s after ${this.state.failures} failures`);
    }
  }
  ...
  async execute<R extends T>(
    ...
    if (this.isOnCooldown()) {
      Logger.log(`[${this.name}] Currently unavailable, ${this.getCooldownRemaining()}s remaining`);
