...
const TECH_FEEDS: Record<string, Feed[]> = {
  ...
  // Tech Silkroads - internet arteries, submarine cables, datacenters, IXPs
  silkroads: [
    { name: 'Internet Silkroads (local)', url: '/api/internet-silkroads' },
    { name: 'Subsea Cable News', url: rss('https://news.google.com/rss/search?q=("submarine+cable"+OR+"undersea+cable"+OR+subsea)+when:14d&hl=en-US&gl=US&ceid=US:en') },
    { name: 'Datacenter & Cloud Regions', url: rss('https://news.google.com/rss/search?q=(datacenter+OR+"cloud+region"+OR+"cloud+infrastructure")+when:14d&hl=en-US&gl=US&ceid=US:en') },
    { name: 'IXP & Peering', url: rss('https://news.google.com/rss/search?q=(IXP+OR+peering+OR+"internet+exchange")+when:14d&hl=en-US&gl=US&ceid=US:en') },
  ],
  regionalStartups: [
    // Europe
