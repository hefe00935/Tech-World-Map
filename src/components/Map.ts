...
export class MapComponent {
  ...
  constructor(container: HTMLElement, initialState: MapState) {
    ...
    // Minimal map chrome for tech/infrastructure view: omit redundant bottom controls, time slider, legend, and timestamp
  ...
  // Controls and time-slider omitted for minimal tech chrome
  ...
  private createLayerToggles(): HTMLElement {
    ...
    const techLayers: (keyof MapLayers)[] = [
      'cables', 'datacenters', 'outages', 'startupHubs', 'cloudRegions', 'accelerators', 'techHQs', 'techEvents', 'silkroads', 'cyberAttacks', 'satellites', 'natural', 'weather', 'economic'
    ];

    const allLayers: (keyof MapLayers)[] = [
      'conflicts','hotspots','sanctions','protests','bases','nuclear','irradiators','military',
      'cables','pipelines','outages','datacenters','ais','flights','natural','weather','economic',
      'waterways','silkroads','spaceports','minerals','fires','ucdpEvents','displacement','climate',
      'startupHubs','cloudRegions','accelerators','techHQs','techEvents','cyberAttacks','satellites'
    ];
    ...
    // Ensure state for each possible layer is present; enable only those we show by default
    allLayers.forEach(l => { if (this.state.layers[l] === undefined) this.state.layers[l] = false; });
    layersToShow.forEach((layer) => {
      this.state.layers[layer] = true;
      const btn = document.createElement('button');
      btn.className = `layer-toggle ${this.state.layers[layer] ? 'active' : ''}`;
      btn.dataset.layer = layer;
      btn.textContent = layerLabels[layer] || String(layer);
  ...
  // Legend, timestamp, and health-check helpers removed for minimal chrome
