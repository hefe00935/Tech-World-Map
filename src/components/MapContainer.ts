...
export class MapContainer {
  ...
  // Show a silkroad route (draw polyline on DeckGL/MapLibre map). For SVG fallback we just center.
  public showSilkroad(route: { id: string; name?: string; points?: [number, number][] } | null): void {
    if (this.useDeckGL) {
      this.deckGLMap?.showSilkroad(route as any);
    } else {
      if (route && route.points && route.points.length > 0) {
        const mid = route.points[Math.floor(route.points.length / 2)];
        if (mid && mid.length >= 2) this.svgMap?.setCenter(mid[1], mid[0]);
      }
    }
  }
  ...
  public setCyberAttacks(attacks: CyberAttack[]): void {
    if (this.useDeckGL) {
      this.deckGLMap?.setCyberAttacks(attacks);
    }
  }
