...
export class MapPopup {
  ...
  private renderDatacenterPopup(dc: AIDataCenter): string {
    ...
    // Very rough heuristic: ~0.1 PFLOPS (FP16/BF16) per high-end accelerator
    const estPflops = (dc.chipCount * 0.1) / 1000;
    const computeLabel = estPflops >= 1 ? `${estPflops.toFixed(1)} PFLOPS` : `${(estPflops * 1000).toFixed(0)} TFLOPS`;

    return `
      <div class="popup-header datacenter ${dc.status === 'planned' ? 'planned' : ''}">
        <span class="popup-title">${escapeHtml(dc.name)}</span>
        <span class="popup-badge ${dc.status}">${dc.status.toUpperCase()}</span>
        <button class="popup-close">×</button>
      </div>
      <div class="popup-body">
        ...
        <div class="popup-stats">
          <div class="popup-stat">
            ...
            <span class="stat-value">${fmt(dc.chipCount)}</span>
          </div>
          <div class="popup-stat">
            <span class="stat-label">EST. COMPUTE</span>
            <span class="stat-value" style="color: #9966ff">${computeLabel}</span>
          </div>
          <div class="popup-stat">
          ...
          <div class="popup-stat">
            ...
            <span class="stat-value">${(dc.chipCount / dc.powerMW).toFixed(0)} c/MW</span>
          </div>
          ` : ''}
          ${dc.sector ? `
          <div class="popup-stat">
        ...
        <div class="popup-attribution">Data: WorldMonitor Intelligence Cluster Alpha</div>
      </div>
    `;
  }
