...
const COLORS = {
  ...
  strategicDataCenter: [0, 255, 200, 180] as [number, number, number, number],
  silkroad: [150, 255, 100, 150] as [number, number, number, number],
...
export class DeckGLMap {
  ...
  private cyberAttackBuffer: Array<CyberAttack & { startTime: number; duration: number }> = [];
  private cyberAnimationRafId: number | null = null;
  ...
  public setCyberAttacks(attacks: CyberAttack[]): void {
    const now = Date.now();
    // Add new attacks to buffer with specialized metadata
    const newAttacks = attacks.map(a => ({
      ...a,
      startTime: now,
      duration: 1500 + Math.random() * 1500 // Randomize travel time between 1.5s - 3s
    }));

    this.cyberAttackBuffer.push(...newAttacks);

    // Keep a maximum buffer to prevent memory issues (e.g., 500 active arcs)
    if (this.cyberAttackBuffer.length > 500) {
      this.cyberAttackBuffer = this.cyberAttackBuffer.slice(-500);
    }

    // Start animation loop if not running
    this.ensureCyberAnimation();
  }

  private ensureCyberAnimation(): void {
    if (this.cyberAnimationRafId !== null) return;

    const animate = () => {
      const now = Date.now();
      const expirationTime = 8000; // Arcs stay for 8 seconds

      // Cleanup expired
      const initialCount = this.cyberAttackBuffer.length;
      this.cyberAttackBuffer = this.cyberAttackBuffer.filter(a => (now - a.startTime) < expirationTime);

      if (this.cyberAttackBuffer.length > 0 || initialCount > 0) {
        this.render(); // Trigger re-render for animation
        this.cyberAnimationRafId = requestAnimationFrame(animate);
      } else {
        this.cyberAnimationRafId = null;
      }
    };
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    // Only populate geopolitical hotspots when not running the tech variant
    this.hotspots = SITE_VARIANT === 'tech' ? [] : [...INTEL_HOTSPOTS];
    ...
    // Minimal map chrome for tech/infrastructure view: keep layer toggles only
  ...
  private renderClusterOverlays(): void {
    ...
    // (no re-declaration) reuse the same activeKeys set

    if (this.state.layers.datacenters && zoom < 5) {
      const clusterRadius = zoom >= 3 ? 30 : zoom >= 2 ? 50 : 70;
      const cacheKey = `dc-${clusterRadius}`;
      let clusters = this.clusterResultCache.get(cacheKey) as Array<{ items: typeof STRATEGIC_DATA_CENTERS; center: [number, number]; screenPos: [number, number] }> | undefined;
      if (!clusters || zoomChanged) {
        const activeDCs = STRATEGIC_DATA_CENTERS.filter(dc => dc.status !== 'decommissioned');
  ...
  private createDatacenterClusterElement(cluster: { items: typeof STRATEGIC_DATA_CENTERS; center: [number, number]; screenPos: [number, number] }): HTMLElement {
    const zoom = this.maplibreMap?.getZoom() || 2;
    ...
    // Removed textContent = '🖥️' to use CSS-styled square
    div.appendChild(icon);
    ...
    if (isCluster) {
      ...
      const existingCount = cluster.items.filter(dc => dc.status === 'existing').length;
      const plannedCount = cluster.items.filter(dc => dc.status === 'planned').length;
      div.title = `${cluster.items.length} Data Centers (${existingCount} active, ${plannedCount} planned)`;
    } else {
      if (zoom >= 3) {
        const label = document.createElement('div');
        label.className = 'datacenter-label';
        label.textContent = primaryDC.name;
        div.appendChild(label);
      }
      div.title = `${primaryDC.name} (${primaryDC.owner})`;
    }
  ...
  private buildLayers(): LayersList {
    ...
    // Earthquakes/Natural events - DISABLED for tech variant
    if (SITE_VARIANT !== 'tech') {
      ...
      // Weather alerts layer (NASA EONET)
    ...
    // Cyber variant / Tech variant auto-start layers
    if (SITE_VARIANT === 'cyber' || SITE_VARIANT === 'tech' || mapLayers.cyberAttacks) {
      if (this.cyberAttackBuffer.length > 0) {
        layers.push(...this.createCyberAttacksLayer());
      }
    }
  ...
  private createDatacentersLayer(): IconLayer {
    ...
    const data = STRATEGIC_DATA_CENTERS.filter(dc => dc.status !== 'decommissioned');
    ...
    return new IconLayer({
      ...
      getColor: (d) => {
        ...
        if (d.status === 'planned') {
          return [136, 68, 255, 80] as [number, number, number, number]; // More subtle for planned
        }
        return [136, 68, 255, 160] as [number, number, number, number]; // Clean purple
      },
      sizeScale: 1,
      sizeMinPixels: 2, // Allow it to shrink very small when zooming out
      sizeMaxPixels: 14,
      pickable: true,
      updateTriggers: {
        getColor: Array.from(highlightedDC),
        getSize: Array.from(highlightedDC),
      }
    });
  }
  ...
  private createCableAdvisoriesLayer(): ScatterplotLayer {
    ...
    return new ScatterplotLayer({
      ...
      getFillColor: (d) => {
        ...
        if (d.severity === 'cyber') {
          return [200, 50, 255, 230] as [number, number, number, number]; // Purple for cyber-related
        }
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'cable-advisories-layer': {
        ...
        const severityLabel = obj.severity === 'cyber' ? '<span style="color:#c832ff">Cyber Risk</span>' : text(obj.severity || 'Advisory');
        return { html: `<div class="deckgl-tooltip"><strong>${text(cableName)}</strong><br/>${severityLabel}<br/>${text(obj.description)}</div>` };
      }
      ...
      case 'satellites-layer':
        return { html: `<div class="deckgl-tooltip"><strong>🛰️ ${text(obj.name)}</strong><br/>Type: ${text(obj.type)}<br/>Owner: ${text(obj.owner)}<br/>Alt: ${Math.round(obj.alt)}km</div>` };
  ...
  // UI Creation methods
  // UI Creation methods (controls/time-slider/legend/timestamp removed for tech-focused minimal chrome)

  private createLayerToggles(): void {
    const toggles = document.createElement('div');
    toggles.className = 'layer-toggles deckgl-layer-toggles';

    // Build complete and tech-focused layer lists; show only tech-focused layers in tech variant
    const allLayers: Array<{ key: keyof MapLayers; label: string; icon: string }> = [
      ...
      { key: 'silkroads', label: 'Silkroads', icon: '&#128268;' },
      { key: 'cyberAttacks', label: 'Cyber Attacks', icon: '&#128293;' },
      { key: 'satellites', label: 'Satellites', icon: '&#128640;' },
    ];

    const techLayers: Array<{ key: keyof MapLayers; label: string; icon: string }> = [
      { key: 'cables', label: 'Undersea Cables', icon: '&#128268;' },
      { key: 'datacenters', label: 'Data Centers', icon: '&#128421;' },
      { key: 'outages', label: 'Internet Outages', icon: '&#128225;' },
      { key: 'startupHubs', label: 'Startup Hubs', icon: '&#128640;' },
    ...
    const layersToShow = SITE_VARIANT === 'tech' ? techLayers : allLayers;

    // Initialize state for all known layers to false, then enable only those we show
    allLayers.concat(techLayers).forEach(l => { this.state.layers[l.key] = this.state.layers[l.key] ?? false; });
    ...
    toggles.innerHTML = `
      ...
      <div class="toggle-list">
        ${layersToShow.map(({ key, label, icon }) => `
    ...
    // Initialize state from toggles and bind change events
  ...
  // Legend/timestamp functions removed to keep minimal chrome for tech variant
  ...
  // resetView removed (unused) to keep minimal chrome
  ...
  // Draw a silkroad route as a GeoJSON LineString on the MapLibre map
  public showSilkroad(route: { id: string; name?: string; points?: [number, number][] } | null): void {
    if (!this.maplibreMap) return;

    // Remove existing source/layer if route is null (clear)
    if (!route) {
      if (this.maplibreMap.getLayer('silkroad-route-line')) this.maplibreMap.removeLayer('silkroad-route-line');
      if (this.maplibreMap.getSource('silkroad-route')) this.maplibreMap.removeSource('silkroad-route');
      return;
    }
    ...
    try {
      if (this.maplibreMap.getSource('silkroad-route')) {
        const src = this.maplibreMap.getSource('silkroad-route') as any;
        src.setData(geojson);
      } else {
        this.maplibreMap.addSource('silkroad-route', { type: 'geojson', data: geojson });
        this.maplibreMap.addLayer({
          id: 'silkroad-route-line',
          type: 'line',
          source: 'silkroad-route',
          layout: { 'line-join': 'round', 'line-cap': 'round' },
          paint: { 'line-color': '#00ccff', 'line-width': 3, 'line-opacity': 0.9 },
        });
      }
      ...
      if (coords.length > 0) {
        ...
        const lons = coords.map(c => c[0]).filter((v): v is number => typeof v === 'number');
        if (lats.length > 0 && lons.length > 0) {
          ...
          const padding = 40;
          this.maplibreMap.fitBounds([[minLon, minLat], [maxLon, maxLat]], { padding });
        }
      }
    } catch (err) {
      console.warn('[DeckGLMap] Failed to show silkroad route:', err);
    }
  }
  ...
  public triggerDatacenterClick(id: string): void {
    const dc = STRATEGIC_DATA_CENTERS.find(d => d.id === id);
    if (dc) {
      // Don't pan - show popup at projected screen position or center
  ...
  private createCyberAttacksLayer(): any[] {

    // 1. Animated "Data Packets" (The "shooting" effect)
    const packetData: any[] = [];
    this.cyberAttackBuffer.forEach(d => {
      const elapsed = Date.now() - d.startTime;
      if (elapsed < 0) return;

      // Each attack "shoots" 3 packets in quick succession
      const packetCount = 3;
      for (let i = 0; i < packetCount; i++) {
        // Offset each packet by a fraction of the duration
        const packetElapsed = elapsed - (i * 400); // 400ms gap between packets
        if (packetElapsed < 0 || packetElapsed > d.duration) continue;

        const t = packetElapsed / d.duration;
        const lng = d.sourceCoords[0] + (d.targetCoords[0] - d.sourceCoords[0]) * t;
        const lat = d.sourceCoords[1] + (d.targetCoords[1] - d.sourceCoords[1]) * t;

        packetData.push({
          position: [lng, lat],
          severity: d.severity,
          alpha: Math.max(0, 255 * (1 - t)) // Fade as it reaches target
        });
      }
    });

    const packetLayer = new ScatterplotLayer({
      id: 'cyber-attack-packets',
      data: packetData,
      getPosition: (d: any) => d.position,
      getFillColor: (d: any) => d.severity === 'critical' ? [255, 255, 255, d.alpha] : [0, 255, 255, d.alpha],
      getRadius: (d: any) => d.severity === 'critical' ? 40000 : 25000,
      radiusMinPixels: 3,
      radiusMaxPixels: 10,
      pickable: false,
    });
