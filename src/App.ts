...
import { fetchEarthquakes, fetchWeatherAlerts, fetchInternetOutages, isOutagesConfigured, fetchAisSignals, initAisStream, getAisStatus, disconnectAisStream, isAisConfigured, fetchCableActivity, fetchProtestEvents, getProtestStatus, fetchFlightDelays, fetchMilitaryFlights, fetchMilitaryVessels, initMilitaryVesselStream, isMilitaryVesselTrackingConfigured, initDB, addToSignalHistory, saveSnapshot, cleanOldSnapshots, fetchNaturalEvents } from '@/services';
...
import { fetchLiveCyberAttacks } from '@/services/cyber-attacks';
...
import { ingestProtestsForCII, ingestMilitaryForCII, ingestOutagesForCII, ingestConflictsForCII, ingestUcdpForCII, ingestHapiForCII, ingestDisplacementForCII, ingestClimateForCII, startLearning, isInLearningMode, calculateCII } from '@/services/country-instability';
...
export class App {
  ...
  constructor(containerId: string) {
    ...
    Logger.log(`[App] Variant check: stored="${storedVariant}", current="${currentVariant}"`);
    if (storedVariant !== currentVariant) {
      // Variant changed - use defaults for new variant, clear old settings
      Logger.log('[App] Variant changed - resetting to defaults');
      ...
      Logger.log('[App] Loaded panel settings from storage:', Object.entries(this.panelSettings).filter(([_, v]) => !v.enabled).map(([k]) => k));
      ...
      if (!localStorage.getItem(PANEL_ORDER_MIGRATION_KEY)) {
        ...
        if (savedOrder) {
          try {
            ...
            Logger.log('[App] Migrated panel order to v1.8 layout');
          } catch {
        ...
        if (!localStorage.getItem(TECH_INSIGHTS_MIGRATION_KEY)) {
          ...
          if (savedOrder) {
            try {
              ...
              newOrder.push(...filtered);
              localStorage.setItem(this.PANEL_ORDER_KEY, JSON.stringify(newOrder));
              Logger.log('[App] Tech variant: Migrated insights panel to top');
            } catch {
              // Invalid saved order, will use defaults
            }
          }
      ...
      // Enforce tech variant defaults: ignore potentially stale saved panels/map layers
      // This ensures the app runs purely in tech mode (no geopolitical panels/layers).
      if (currentVariant === 'tech') {
        Logger.log('[App] Enforcing tech variant defaults - overriding saved panels and layers');
        this.mapLayers = { ...defaultLayers };
        this.panelSettings = { ...DEFAULT_PANELS };
  ...
  public async init(): Promise<void> {
    ...
    if (titleEl) {
      titleEl.textContent = SITE_VARIANT === 'cyber' ? 'Cyber Monitör' : SITE_VARIANT === 'tech' ? 'Cyber Command' : 'World Monitor';
    }
  ...
  private setupMapLayerHandlers(): void {
    this.map?.setOnLayerChange((layer, enabled) => {
      Logger.log(`[App.onLayerChange] ${layer}: ${enabled}`);
      // Save layer settings
      ...
      if (enabled) {
        ...
        if (layer === 'cyberAttacks') {
          this.loadDataForLayer('cyberAttacks');
        } else if (layer === 'satellites') {
          this.loadDataForLayer('satellites');
        } else {
  ...
  private setupCountryIntel(): void {
    ...
    this.map.onCountryClicked(async (lat, lon) => {
      ...
      try {
        ...
        Logger.error('[CountryIntel] fetch error:', err);
  ...
  private setupSearchModal(): void {
    ...
    if (SITE_VARIANT === 'tech') {
      ...
      // Satellites
      this.searchModal.registerSource('satellite', [
        { id: 'sat-all', title: 'Live Satellites', subtitle: 'Space Situational Awareness', data: {} }
      ]);

      // Cyber Attacks (Generic search entry as we don't index live attacks)
      this.searchModal.registerSource('cyber', [
        { id: 'cyber-live', title: 'Live Cyber Attacks', subtitle: 'Real-time threat monitoring', data: {} }
      ]);

      // Major Tech Infrastructure Providers (Cloud/HQs)
      this.searchModal.registerSource('techhq', TECH_HQS.map(h => ({
        id: h.id,
        title: h.company,
        subtitle: `${h.type === 'faang' ? 'Cloud/Infra Provider' : 'Tech Giant'} • ${h.city}, ${h.country}`,
        data: h,
      })));

      // Keep key companies but frame as infrastructure/key players
      this.searchModal.registerSource('techcompany', TECH_COMPANIES.map(c => ({
        id: c.id,
        title: c.name,
        subtitle: `${c.sector} • Key Infrastructure Player`,
        data: c,
      })));
    ...
    this.container.innerHTML = `
      <div class="header">
        <div class="header-left">
          <div class="variant-switcher">
            <span class="variant-option active" data-variant="tech" title="Tech & AI Intelligence (current)">
              <span class="variant-icon">💻</span>
              <span class="variant-label">TECH</span>
            </span>
          </div>
          <span class="logo">VIEWER</span><span class="version">v${__APP_VERSION__}</span>
        ...
        <div class="header-center">
          <a class="join-btn" id="joinServerBtn" href="https://discord.gg/PeyQxeGkJQ" target="_blank" rel="noopener noreferrer" title="Join Server">
            <span class="join-icon" aria-hidden="true">
              <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
                <path d="M20 8.5c-.6-.3-1.2-.5-1.8-.6.1-.3.1-.6.1-.9 0-2.6-2.1-4.7-4.7-4.7-1 0-1.9.3-2.7.8-.8-.5-1.7-.8-2.7-.8-2.6 0-4.7 2.1-4.7 4.7 0 .3 0 .6.1.9-.6.1-1.2.3-1.8.6-.3.2-.5.6-.5 1 0 6.1 3.9 9.9 9.6 9.9s9.6-3.8 9.6-9.9c0-.4-.2-.8-.5-1zM9.8 12.8c-1 0-1.8-.9-1.8-2s.8-2 1.8-2 1.8.9 1.8 2-.8 2-1.8 2zm4.4 0c-1 0-1.8-.9-1.8-2s.8-2 1.8-2 1.8.9 1.8 2-.8 2-1.8 2z"/>
              </svg>
            </span>
            <span class="join-label">Join Server</span>
          </a>
        </div>
      ...
      <div class="main-content">
        <div class="map-section" id="mapSection">
          ...
          <div class="branding-bottom-right">Made By Malvionix</div>
  ...
  private createPanels(): void {
    const panelsGrid = document.getElementById('panelsGrid') as HTMLElement;
    ...
    if (SITE_VARIANT !== 'tech') {
      ...
      // Keep AI panel for tech variant
      const aiPanel = new NewsPanel('ai', 'AI / ML');
      this.attachRelatedAssetHandlers(aiPanel);
      this.newsPanels['ai'] = aiPanel;
      this.panels['ai'] = aiPanel;
    ...
    // Tech variant panels - Disable most startup/VC panels for infrastructure focus
    if (SITE_VARIANT !== 'tech') {
      ...
      const trendsPanel = new NewsPanel('github', 'Tech Trends');
      this.attachRelatedAssetHandlers(trendsPanel);
      this.newsPanels['github'] = trendsPanel;
      this.panels['github'] = trendsPanel;
      ...
      // For Tech Variant: Keep only Infrastructure/Security related panels
      const securityPanel = new NewsPanel('security', 'Cybersecurity');
      this.attachRelatedAssetHandlers(securityPanel);
      this.newsPanels['security'] = securityPanel;
      this.panels['security'] = securityPanel; // ID: security

      const hardwarePanel = new NewsPanel('hardware', 'Semiconductors & Hardware');
      this.attachRelatedAssetHandlers(hardwarePanel);
      this.newsPanels['hardware'] = hardwarePanel;
      this.panels['hardware'] = hardwarePanel; // ID: hardware

      const cloudPanel = new NewsPanel('cloud', 'Cloud & Infrastructure');
      this.attachRelatedAssetHandlers(cloudPanel);
      this.newsPanels['cloud'] = cloudPanel;
      this.panels['cloud'] = cloudPanel; // ID: cloud
    }
    ...
    if (SITE_VARIANT === 'full') {
      ...
      strategicPosturePanel.setLocationClickHandler((lat, lon) => {
        Logger.log('[App] StrategicPosture handler called:', { lat, lon, hasMap: !!this.map });
        this.map?.setCenter(lat, lon, 4);
      });
    ...
    // Live News (Video) - Disable for Tech/Infra variant
    if (SITE_VARIANT !== 'tech') {
    ...
    // Silkroads (tech infra) panel - only initialize for tech variant
    if (SITE_VARIANT === 'tech') {
      import('./components/SilkroadsPanel')
        .then((mod) => {
          const SilkroadsPanel = mod.SilkroadsPanel;
          return import('./services/internet-silkroads').then((svc) => ({ SilkroadsPanel, svc }));
        })
        .then(({ SilkroadsPanel, svc }) => {
          try {
            const silkPanel = new SilkroadsPanel();
            silkPanel.setOnRouteClick((route: any) => {
              // Draw route on the map and fit bounds; keep visible until user clears
              this.map?.showSilkroad(route);
            });
            silkPanel.setRoutes(svc.getInternetSilkroads());
            this.panels['silkroads'] = silkPanel;
          } catch (err) {
            console.warn('Silkroads initialization error:', err);
          }
        })
        .catch((err) => {
          console.warn('Failed to lazy-load SilkroadsPanel or service:', err);
        });
    }
    ...
    if (this.panels['live-news']) {
      ...
      } else if (liveNewsIdx === -1) {
        // If enabled but not in order (e.g. fresh load), prepend it
        // Check if it's meant to be enabled first
        if (DEFAULT_PANELS['live-news']?.enabled) {
          panelOrder.unshift('live-news');
        }
      }
    }
  ...
  private async loadAllData(): Promise<void> {
    ...
    if (SITE_VARIANT === 'cyber' || SITE_VARIANT === 'tech' || this.mapLayers.cyberAttacks) {
      tasks.push({ name: 'cyberAttacks', task: runGuarded('cyberAttacks', () => this.loadCyberAttacks()) });
    }
  ...
  private async loadCyberAttacks(): Promise<void> {
    try {
      const attacks = await fetchLiveCyberAttacks();
      this.map?.setCyberAttacks(attacks);
      this.statusPanel?.updateFeed('CyberMap', { status: 'ok', itemCount: attacks.length });
    } catch (e) {
      console.warn('[App] Cyber attacks load failed:', e);
      this.statusPanel?.updateFeed('CyberMap', { status: 'error' });
    }
  }
  ...
  private setupRefreshIntervals(): void {
    ...
    this.scheduleRefresh('cyberAttacks', () => this.loadCyberAttacks(), 2000, () => SITE_VARIANT === 'tech' || !!this.mapLayers.cyberAttacks);
  }
}
