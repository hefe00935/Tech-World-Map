  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        Logger.warn(`[Military Flights] Rate limited for ${hotspot.name}`);
      }
      return [];
    }
...
async function fetchFromOpenSky(): Promise<MilitaryFlight[]> {
  ...
  Logger.log(`[Military Flights] Found ${allFlights.length} military aircraft from ${MILITARY_HOTSPOTS.length} regions`);
  return allFlights;
}
...
async function enrichFlightsWithWingbits(flights: MilitaryFlight[]): Promise<MilitaryFlight[]> {
  ...
  if (!isConfigured) {
    Logger.log('[Military Flights] Wingbits not configured, skipping enrichment');
    return flights;
  }
  ...
  Logger.log(`[Military Flights] Enriching ${detailsMap.size} of ${flights.length} aircraft with Wingbits data`);
...
export async function fetchMilitaryFlights(): Promise<{
  ...
  return breaker.execute(async () => {
    ...
    Logger.log(`[Military Flights] Total: ${flights.length} flights, ${clusters.length} clusters`);
    return { flights, clusters };
  }, { flights: [], clusters: [] });
}
