...
function processAisPosition(data: AisPositionData): void {
  ...
  if (previousSize === 0 || (trackedVessels.size === 10 && previousSize < 10)) {
    vesselCache = null;
    breaker.clearCache();
    Logger.log(`[Military Vessels] First vessels arriving (tracked: ${trackedVessels.size})`);
  }

  if (messageCount % 50 === 0) {
    Logger.log(`[Military Vessels] Tracking ${trackedVessels.size} military/gov vessels`);
  }
}
...
export function initMilitaryVesselStream(): void {
  ...
  Logger.log('[Military Vessels] Initializing tracking via shared AIS stream');
