...
class ParallelAnalysisService {
  ...
  async analyzeHeadlines(clusters: ClusteredEvent[]): Promise<AnalysisReport> {
    ...
    Logger.log(`🔬 Parallel Analysis #${this.analysisCount}`);
    Logger.log(`Analyzing ${clusters.length} headlines...`);
    ...
    if (mlWorker.isAvailable) {
      ...
      Logger.log('✓ ML models loaded');
      if (entities) Logger.log(`  → NER extracted ${entities.flat().length} entities`);
      if (embeddings) Logger.log(`  → Embeddings computed for ${embeddings.length} headlines`);
    } else {
      Logger.log('⚠ ML not available, using keyword-only analysis');
    }
  ...
  private logReport(report: AnalysisReport, durationMs: number): void {
    Logger.log(`Analysis completed in ${durationMs.toFixed(0)}ms`);

    Logger.log('TOP BY CONSENSUS (high confidence)');
    Logger.table(report.topByConsensus.slice(0, 5).map(h => ({
    ...
    if (report.topByDisagreement.length > 0) {
      Logger.log('HIGH DISAGREEMENT (perspectives conflict)');
      Logger.table(report.topByDisagreement.map(h => ({
    ...
    if (report.missedByKeywords.length > 0) {
      Logger.log('POTENTIALLY MISSED (ML says important, keywords say no)');
      report.missedByKeywords.forEach(h => {
        ...
        Logger.log(`  "${h.title.slice(0, 70)}..."`);
        Logger.log(`    keywords: ${(keywordScore * 100).toFixed(0)}% | ML avg: ${(mlScores.reduce((s, p) => s + p.score, 0) / mlScores.length * 100).toFixed(0)}%`);
        mlScores.forEach(p => {
          Logger.log(`      ${p.name}: ${(p.score * 100).toFixed(0)}% - ${p.reasoning}`);
        });
      });
    }

    Logger.log('Perspective Correlations');
    const sortedCorr = Object.entries(report.perspectiveCorrelations)
      .sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]));
    sortedCorr.slice(0, 5).forEach(([pair, corr]) => {
      Logger.log(`  ${pair}: ${corr.toFixed(2)}`);
    });
  }
