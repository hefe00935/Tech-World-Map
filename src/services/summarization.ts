...
async function tryGroq(headlines: string[], geoContext?: string): Promise<SummarizationResult | null> {
  ...
  try {
    ...
    Logger.log(`[Summarization] ${provider === 'cache' ? 'Redis cache hit' : 'Groq success'}:`, data.model);
    return {
    ...
    Logger.warn('[Summarization] Groq failed:', error);
    return null;
  }
}
...
async function tryOpenRouter(headlines: string[], geoContext?: string): Promise<SummarizationResult | null> {
  ...
  try {
    ...
    Logger.log(`[Summarization] ${provider === 'cache' ? 'Redis cache hit' : 'OpenRouter success'}:`, data.model);
    return {
      summary: data.summary,
      provider: provider as SummarizationProvider,
      cached: !!data.cached,
    ...
    Logger.warn('[Summarization] OpenRouter failed:', error);
    return null;
  }
}
...
async function tryBrowserT5(headlines: string[]): Promise<SummarizationResult | null> {
  try {
    if (!mlWorker.isAvailable) {
      Logger.log('[Summarization] Browser ML not available');
      return null;
    }
    ...
    Logger.log('[Summarization] Browser T5 success');
    return {
      summary,
      provider: 'browser',
      cached: false,
    ...
    Logger.warn('[Summarization] Browser T5 failed:', error);
    return null;
  }
}
...
export async function generateSummary(
  ...
  Logger.warn('[Summarization] All providers failed');
  return null;
}
