...
export async function checkWingbitsStatus(): Promise<boolean> {
  ...
  try {
    ...
    Logger.log(`[Wingbits] Status: ${wingbitsConfigured ? 'configured' : 'not configured'}`);
...
export async function getAircraftDetailsBatch(icao24List: string[]): Promise<Map<string, WingbitsAircraftDetails>> {
  ...
  try {
    ...
    Logger.log(`[Wingbits] Batch: ${results.size} enriched, ${data.cached || 0} cached, ${data.fetched || 0} fetched`);
    if (results.size > 0) {
      dataFreshness.recordUpdate('wingbits', results.size);
    }
  } catch (error) {
    Logger.warn('[Wingbits] Batch fetch failed:', error);
