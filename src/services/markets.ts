...
async function fetchFromFinnhub(
  ...
  try {
    ...
    if (!response.ok) {
      Logger.warn(`[Markets] Finnhub returned ${response.status}`);
      return [];
    }
    ...
    if (data.error) {
      Logger.warn(`[Markets] Finnhub error: ${data.error}`);
      return [];
    }
    ...
    Logger.error('[Markets] Finnhub fetch failed:', error);
    return [];
  }
}
...
export async function fetchCrypto(): Promise<CryptoData[]> {
  try {
    ...
    Logger.error('Failed to fetch crypto:', e);
    return [];
  }
}
