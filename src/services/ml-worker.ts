...
class MLWorkerManager {
  ...
  async init(): Promise<boolean> {
    ...
    if (!this.capabilities.isSupported) {
      Logger.log('[MLWorker] ML features disabled (device not supported)');
      return false;
    }
  ...
  private initWorker(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      const readyTimeout = setTimeout(() => {
        if (!this.isReady) {
          Logger.error('[MLWorker] Worker failed to become ready');
          this.cleanup();
          resolve(false);
        }
      ...
      try {
        ...
        Logger.error('[MLWorker] Failed to create worker:', error);
        this.cleanup();
        resolve(false);
        return;
      }
      ...
      this.worker.onmessage = (event: MessageEvent<WorkerResult>) => {
        ...
        if (data.type === 'error') {
          ...
          if (pending) {
            ...
            Logger.error('[MLWorker] Error:', data.error);
          }
          return;
        }
      ...
      this.worker.onerror = (error) => {
        Logger.error('[MLWorker] Error:', error);
