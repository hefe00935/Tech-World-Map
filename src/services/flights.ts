...
async function fetchFAADelays(): Promise<Map<string, FAADelayInfo>> {
  ...
  try {
    ...
    if (!response.ok) {
      Logger.warn(`[Flights] FAA NASSTATUS error: ${response.status}`);
      return new Map();
    }
    ...
    Logger.log(`[Flights] FAA reports ${delays.size} airports with delays`);
    return delays;
  } catch (error) {
    Logger.error('[Flights] Failed to fetch FAA NASSTATUS:', error);
    return new Map();
  }
}
