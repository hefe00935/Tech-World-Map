...
class SignalAggregator {
  ...
  generateStrategicContext(): string {
...
export function logSignalSummary(): void {
  ...
  Logger.log('[Signal Aggregator]');
  Logger.log(`Total signals: ${summary.totalSignals}`);
  Logger.log('By type:', summary.byType);

  if (summary.convergenceZones.length > 0) {
    Logger.log('Convergence Zones:');
    for (const z of summary.convergenceZones) {
      Logger.log(`  ${z.description}`);
    }
  }

  if (summary.topCountries.length > 0) {
    Logger.log('Top Countries:');
    for (const c of summary.topCountries.slice(0, 5)) {
      Logger.log(`  ${c.countryName}: ${c.totalCount} signals, score ${c.convergenceScore}`);
    }
  }
}
