...
class FocalPointDetector {
  ...
  analyze(clusters: ClusteredEvent[], signalSummary: SignalSummary): FocalPointSummary {
    ...
    const strategicContext = this.generateStrategicContext(focalPoints);
   ...
   */
  private generateStrategicContext(focalPoints: FocalPoint[]): string {
  ...
  logSummary(): void {
    if (!this.lastSummary) {
      Logger.log('[FocalPointDetector] No summary available');
      return;
    }

    Logger.log('[FocalPointDetector]');
    Logger.log(`Total focal points: ${this.lastSummary.focalPoints.length}`);
    ...
    if (critical.length > 0) {
      Logger.log('Critical:');
      for (const fp of critical) {
        Logger.log(`  ${fp.displayName}: score ${fp.focalScore.toFixed(0)}, ${fp.newsMentions} news, ${fp.signalCount} signals`);
      }
    }
    ...
    if (elevated.length > 0) {
      ...
      for (const fp of elevated.slice(0, 5)) {
        Logger.log(`  ${fp.displayName}: score ${fp.focalScore.toFixed(0)}`);
      }
    }
  }
}
